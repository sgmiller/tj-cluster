...
uint8_t Instrument::GetByte(int bpos) { return _message[bpos]; }
...
void Instrument::setWriter(InstrumentWriter *writer) { _writer = writer; }

InstrumentWriter::InstrumentWriter(Instrument **instruments,
                                   int instrumentCount)
{
...
void SingleLamp::SetLamp(bool on) { SetByte(1, lampBool(on)); }
...
void Odometer::AddFeet(int feet) { AddMiles(feet / 5280.0); }
